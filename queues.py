...
class CircularQueue:
    ...
    def enqueue(self, item): 
       ...
       print(len(self.__items) < self.__capacity)
       if len(self.__items) < self.__capacity:
        print("first block")
        self.__items.append(item)
       else:        
...
def main():
    ...
    print("There are "+str(bq.size())+" elements in my bounded queue.")
    print('----------------------------------')
    ...
    element = bq.dequeue()
    print(repr(bq))
    print(element+" was first in the bounded queue: "+str(bq))
    print("There are "+str(bq.size())+" elements in my bounded queue.")   
    print('----------------------------------')   
    ...
    cq = CircularQueue(3)
    index = 0
    # checking whether the program raises an error while dequeuing from empty stack.
    # cq.dequeue()
    # enqueue operation performed on circular queue.
    index = 0
    while index < cq.capacity():
        get_item = input("Enter something to be enetered in the queue: ")
        cq.enqueue(get_item)
        index += 1
    print(repr(cq))
    # dequeueing item from the queue
    cq.dequeue()
    cq.dequeue()
    cq.dequeue()
    print(repr(cq))
